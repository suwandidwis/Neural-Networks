function [finalerror,v,w,v0,w0,Y,epoch] = backpropagationIDCoba(u_k,y_k,alpha,lh,momentum)
...
input(1,:)=[0 0 0 0 0 0];
input(2,:)=[0 0 u_k(1) 0 0 target(1)];
input(3,:)=[0 u_k(1) u_k(2) 0 target(1) target(2)];
for i = 4 : 1000
    input(i,:) = [u_k(i-3) u_k(i-2) u_k(i-1) target(i-3) target(i-2) target(i-1)];
end
...
while MSSE > 0.00001
    ...
    MSSE=sum(error)/N
    time=toc
    save('Input_NNID21.mat','x','t','alpha','lh','momentum');
    save('Hasil_NNID21.mat','MSSE','errortotal','v','w','v0','w0','v1','w1','v00','w00','Y','epoch','time');
end
