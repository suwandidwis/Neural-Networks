function [finalerror,v,w,v0,w0,Y,epoch] = backpropagation(x,alpha,t,lh,momentum)
...
while MSSE > 0.01
    ...
    %Backpropagation of error dari bobot w
        ...
        deltaw0 = alpha * d(j);                     %menghitung besarnya koreksi bias output
        for i = 1 : lh
            d_in(i) = d(j) * w(j);                    %menghitung semua koreksi error
            d(i) = d_in(i) * z(j)*(1-z(j));           %menghitung nilai aktivasi koreksi error
            deltav(:,i) = alpha * d(i) * x(i);          %menghitung koreksi bobot unit hidden      
        end
        deltav0 = alpha * d(i);                          %menghitung koreksi error bias unit hidden
